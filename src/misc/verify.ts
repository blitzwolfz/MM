...
//                     const filter = (m: Discord.Message) => m.content.includes(id);

//                     await message.reply("Code has been sent to your reddit dm. Please do `!code <your code>` to verify! You only get one chance at it!")
//                         .then(
//                             async () =>
//                                 await message.channel.awaitMessages(filter, { max: 1, time: 1200000, errors: ['time'] })
//                                     .then(async () => {


//                                         (await   (await guild.members.fetch(message.author.id)).setNickname(userInfo.name));
//                                         (await   (await guild.members.fetch(message.author.id))!.roles.remove("730650583413030953"));
//                                         (await   (await guild.members.fetch(message.author.id))!.roles.add("719941380503371897"));
//                                         //(await guild.members.fetch(message.author.id))!.roles.remove("730650583413030953")

//                                         (await guild.members.fetch(message.author.id))

//                                         await message.author.send("Remember to check <#722284401920180234>, <#722284266108747880>, <#722284377609994281>, and to signup for both vote pings and signup pings in <#731568511138136094>! Enjoy your stay.")
