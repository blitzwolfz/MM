...
async function modLb(page: number = 1, client: Discord.Client, ratings: modprofile[], args:string, rest:string){
    ...
    return {
        ...
        description: fields.length === 0 ?
            ...
            `Your rank is: ${ratings.findIndex(item => item._id === rest) + 1}`,
        fields,
        color: "#d7be26",
        timestamp: new Date()
    };
...
export async function clearmodstats(message: Discord.Message){
    ...
    let profiles = await getAllModProfiles("matchportionsstarted")
